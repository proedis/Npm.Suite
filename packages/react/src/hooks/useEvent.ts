...
interface UseEventOptions {
  ...
  /** Change the event target, default to the main document element */
  target?: HTMLElement;
}
...
export function useEvent<K extends keyof HTMLElementEventMap>(
   ...
   * This logic will help the event listener to get the last
   * version update of the handler, without having the need to save it
  ...
  useEnhancedEffect(
    () => {
       ...
       * could not be the same reference as the component mount
       */
      ...
      /** On effect clear, remove the event attached to the target */
      return () => {
