...
| Change | What to do |
| --- | --- |
| `useDataSelector` reconciles during render | Nothing, in most cases — and one render fewer per data change. The selection was previously corrected by an effect calling `setState`, so a render could show a selection the data no longer backed. `onSelectedChange` now fires from a single place, **after** the commit, and it also fires when a data change invalidates the selection — which it used to miss entirely. |
