...
import { areArrayStrictEquals } from '@proedis/utils';


/* --------
 * Internal Interface
 * -------- */
export interface UseDataSelectorConfig<Data extends AnyObject> {
  ...
  /** A handler to call when selected data change */
...
type UserDataSelectorConfigAndData<Data extends AnyObject> = UseDataSelectorConfig<Data> & {
  /** All Data List */
  allData: Data[];
 ...
 * Hook Return Type
 * -------- */
export interface DataSelector<Data extends AnyObject> {
 ...
 * -------- */
export default function useDataSelector<Data extends AnyObject>(
  ...
  if (process.env.NODE_ENV === 'development' && selectable) {
    invariant(
      ...
      'To correctly use selectable table, the getRowKey function must be declared'
    );
  }
  ...
  const dataKeys: Map<Data, React.Key> = React.useMemo(
    () => {
      ...
      /** Loop each row and get its own key */
  ...
  const getSelectedData = React.useCallback(
    (currentSelected: React.Key[] = selectedKeys) => (
      allData.filter((row) => {
        /** Get the row key */
        const key = dataKeys.get(row);
        /** Return key exists and is included in the selectedKeys */
  ...
  React.useEffect(
    () => {
      ...
      if (!areArrayStrictEquals(newSelected, selectedKeys)) {
        setSelectedKeys(() => {
          handleSelectedDataChange(newSelected);
          return newSelected;
        });
